    ...
    pub async fn async_pull_layers(
        ...
        let layer_metas = stream::iter(layer_descs)
            ...
            .map(|(i, layer)| {
                ...
                async move {
                    println!("CSG-M4GIC: B3G1N: Pull Single Layer");
                    let layer_reader = client
                        .async_pull_blob(reference, &layer.digest)
                        .await
                        .map_err(|e| anyhow!("failed to async pull blob {}", e.to_string()))?;
                    println!("CSG-M4GIC: END: Pull Single Layer");

                    println!("CSG-M4GIC: B3G1N: Handle Single Layer");
                    ...
                    .await;
                    println!("CSG-M4GIC: END: Handle Single Layer");
